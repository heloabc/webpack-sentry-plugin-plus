...
module.exports = class SentryPlugin {
  ...
  apply(compiler) {
    compiler.hooks.done.tapAsync('SentryPlus', (stats, cb) => {
      ...
      }).then(() => cb()).catch(err => this.handleErrors(err, compilation, cb));
    });
  }
  ...
  getFiles(compilation) {
    return Object.keys(compilation.assets).map(name => {
      if (this.isIncludeOrExclude(name)) {
        return {
          name,
          path: fspath.join(compilation.outputOptions.path, name)
        };
      }
  ...
  uploadFileWithRetry(obj) {
    return (0, _asyncToGenerator2.default)(function* () {
      let tryCount = 0;

      while (tryCount < 3) {
        try {
          yield this.uploadFile(obj);
  ...
  deleteFiles(stats) {
    Object.keys(stats.compilation.assets).filter(name => this.deleteRegex.test(name)).forEach(name => {
